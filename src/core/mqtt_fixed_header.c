...
void mqtt_ctl_head_pack_s(struct mqtt_ctl_head * p_header){
	uint8_t head = 0;
        //!< check parameters
	MQTT_CTL_TYPE_CHECK(p_header->type);
	MQTT_CTL_FLAG_DUP_CECKE(p_header->DUP);
...
int mqtt_ctl_encode_remaining_len(const uint8_t * code, int length){
	int count = 0;
	do{
		*code = length % 128;
		length /= 128;
