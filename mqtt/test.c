...
void test_fixed_header(void) {
    ...
    printf("eval:all=0x%2x,type=%d,DUP=%d,QoS=%d,RETAIN=%d\n", flag2.all,
           flag2.bits.type, flag2.bits.DUP, flag2.bits.QoS, flag2.bits.RETAIN);
    mqtt_buf_release(p_buf_ctl_flag);

    //!< control packet header remaining length
    mqtt_attr_re_len_t length[] = {0,     127,     128,     16383,
    ...
    for (int i = 0; i < COUNT_OF_ARRAY(length); i++) {
        ...
        struct mqtt_buf_re_len* mq_buf_re_len =
            mqtt_ctl_encode_remaining_len(length[i]);
...
void test_var_header(void) {
    ...
    union mqtt_attr_connect_flag conn_flag = {.bits = {
                                                  0,  //!< flag reserved
    ...
    struct mqtt_buf_connect_flag* p_conn_buf_flag =
        mqtt_connect_flag_pack(conn_flag);
    union mqtt_attr_connect_flag conn_attr_flag =
        mqtt_connect_flag_unpack(p_conn_buf_flag);
    ...
    printf("evaluate vlaue of flags by order = %2x,%2x,%2x,%2x,%2x,%2x,%2x\n",
           conn_attr_flag.bits.flag_user_name, conn_attr_flag.bits.flag_pwd,
           conn_attr_flag.bits.flag_w_retain, conn_attr_flag.bits.flag_w_QoS,
    ...
    union mqtt_attr_connack_flag connack_flag = {.bits = {1, 0}};
    struct mqtt_buf_connack_flag* p_buf_connack_flag =
        mqtt_connack_flag_pack(connack_flag);
    union mqtt_attr_connack_flag connack_attr_flag =
        mqtt_connack_flag_unpack(p_buf_connack_flag);
    ...
    printf("evaluete value of flags by order = 0x%2x,0x%2x\n",
           connack_attr_flag.bits.reserved, connack_attr_flag.bits.SP);
...
void test_payload(void) {
    ...
    union mqtt_attr_payload_suback_ret_code suback_ret_code = {.bits = {
                                                                   2,  //!< QoS
                                                                   0,
                                                                   1,  //!< ok
                                                               }};
    ...
    printf(
        "evaluate value of suback ret_codes `all=0x%2x` `ok=%2x` , `QoS=%2x`\n",
        suback_attr_ret_code.all, suback_attr_ret_code.bits.ok,
    ...
    printf(
        "evaluate value of subscribe content flags "
        "`reserved=0x%2x`,`QoS=0x%2x`",
        subscribe_attr_flag.bits.reserved, subscribe_attr_flag.bits.QoS);
...
void test_packet(void) {
    ...
    */
    ...
    // mqtt_log_print_buf(LOG_LEVEL_LOG, attr_connect->payload->buf,
    // attr_connect->payload->len_valid);
    //!< w_topic
...
void test_packet_suback(void) {
    ...
    //< payload
    mqtt_attr_payload_suback_ret_code_t ret_codes[2] = {{.bits =
                                                             {
                                                                 .ok = 1,
                                                                 .reserved = 0,
                                                                 .QoS = 1,
    ...
    mqtt_attr_payload_write_bytes(attr_suback->payload, (uint8_t*)ret_codes,
                                  sizeof(ret_codes));
...
void test_packet_disconnect(void) {
    ...
    //< filling packet attributes
    //< hdr
    attr_disconnect->hdr.bits.DUP = 1;
    attr_disconnect->hdr.bits.QoS = 0;
    attr_disconnect->hdr.bits.RETAIN = 1;
    attr_disconnect->hdr.bits.type = MQTT_CTL_TYPE_DISCONNECT;
    //< variable
    //< payload

    //< pack packet
    mqtt_buf_packet_t* buf_packet;
    mqtt_pack_disconnect(attr_disconnect, &buf_packet);
    //< unpack packet
    mqtt_attr_packet_t* attr_packet;
    mqtt_unpack_disconnect(buf_packet, &attr_packet);
    printf("hdr = `0x%2x`\n", attr_packet->hdr.all);
    printf("remaining length = `0x%u`\n", attr_packet->remaining_length);

    mqtt_attr_packet_release(attr_disconnect);
    mqtt_attr_packet_release(attr_packet);
