...
int mqttor_client_connect(mqttor_session_t* mq_sess, const char* host,
    ...
    */
    ...
    if (0 > mq_sess->socket) {  //!< invalid socket
        if (0 > (mq_sess->socket =
                     socket(addr.sin_family, SOCK_STREAM, IPPROTO_TCP))) {
            mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor create socket fail!\n");
            return -E_NET_SOCK;
        }
    }
    ...
    //< mqtt connect
    mqtt_attr_packet_t* p_attr_connect =
        mqtt_attr_packet_new(MQTT_FIXED_PAYLOAD_LEN_CONNECT);
    ...
    p_attr_connect->attr_packet.connect.flag.bits.flag_w_flag =
        mq_sess->config->will_message && mq_sess->config->will_topic ? true
                                                                     : false;
    ...
    err = mqtt_pack_connect(p_attr_connect, &p_buf_packet);
    if (0 > err) {
    ...
    if (0 > err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor send connect packet fail!\n");
        return -E_NET_CONN;
    }
    ...
    if (MQTT_CTL_TYPE_CONNACK == p_attr_connack->hdr.bits.type) {
        if (CONNECT_RET_CODE_ACCEPTED ==
            p_attr_connack->attr_packet.connack.ret_code) {
            mqtt_log_printf(LOG_LEVEL_LOG, "Mqttor connect to broker ok!\n");
            err = E_NONE;
        } else {
            mqtt_log_printf(
                LOG_LEVEL_LOG,
        ...
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor connect to broker fail!\n");
        err = -E_NET_CONN;
    }
...
int mqttor_client_disconnect(mqttor_session_t* mq_sess) {
    ...
    mqtt_buf_packet_t* p_buf_packet = NULL;
    err = mqtt_pack_disconnect(p_attr_packet, &p_buf_packet);
    if (0 > err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor packet disconnect fail!\n");
        return err;
    }
    err = send(mq_sess->socket, p_buf_packet->buf, p_buf_packet->len, 0);
    if (0 > err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor send disconnect packet fail!\n");
        return -E_NET_CONN;
    }
    mqtt_buf_release(p_buf_packet);
...
int mqttor_client_publish(mqttor_session_t* mq_sess, /*const*/ char* topic,
    ...
    //!< mqtt publish
    mqtt_attr_packet_t* p_attr_publish =
        mqtt_attr_packet_new(payload->len_valid);
    ...
    //!< handle puback
    mqtt_buf_packet_t* p_buf_puback =
        mqtt_buf_new(MQTT_FIXED_PACKET_LEN_PUBACK);
    mqtt_attr_packet_t* p_attr_puback = NULL;  // mqtt_attr_packet_new(0);
    switch (qos) {
        case MQTTOR_QoS_MONCE:
            //< no puback
            err = E_NONE;
            mqtt_log_printf(LOG_LEVEL_LOG, "Mqttor publish ok!\n");
            break;
        case MQTTOR_QoS_LONCE:
            //< puback
            err =
                recv(mq_sess->socket, p_buf_puback->buf, p_buf_puback->len, 0);
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor recv puback fail!\n");
                break;
            }
            err = mqtt_unpack_puback(p_buf_puback, &p_attr_puback);
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor unpack puback fail!\n");
                break;
            }
            if (MQTT_CTL_TYPE_PUBACK == p_attr_puback->hdr.bits.type) {
                ...
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor publish fail!\n");
                err = -E_NET_CONN;
                break;
            }
            break;
        case MQTTOR_QoS_EONCE:
            ...
            err =
                recv(mq_sess->socket, p_buf_puback->buf, p_buf_puback->len, 0);
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor recv pubrec fail!=n");
                break;
            }
            err = mqtt_unpack_pubrec(p_buf_puback, &p_attr_puback);
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor unpack pubrec fail!\n");
                break;
            }
            ...
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor pack pubrel fail!\nn");
                break;
            }
            err =
                send(mq_sess->socket, p_buf_puback->buf, p_buf_puback->len, 0);
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor send pubrel fail!\n");
                err = -E_NET_XFER;
                break;
            }
            //< pubcomp
            err =
                recv(mq_sess->socket, p_buf_puback->buf, p_buf_puback->len, 0);
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor recv pubcomp fail!=n");
                err = -E_NET_XFER;
                break;
            }
            ...
            if (0 > err) {
                mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor unpack pubcomp fail!\n");
                break;
            }
            ...
            break;
        default:
            mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor unkown qos `%d`!\n", qos);
            err = -E_SESS_ACK;
    }
...
int mqttor_client_subscribe(mqttor_session_t* mq_sess, const char* sub,
    ...
    mqtt_attr_packet_t* p_attr_subscribe =
        mqtt_attr_packet_new(len_sub + MQTT_BUF_STR_MAX_BYTE + sizeof(uint8_t));
    p_attr_subscribe->hdr.bits.type = MQTT_CTL_TYPE_SUBSCRIBE;
    p_attr_subscribe->attr_packet.subscribe.id_packet = mq_sess->id_packet++;
    mqtt_attr_payload_write_string(p_attr_subscribe->payload, sub);
    mqtt_attr_payload_write_byte(
        p_attr_subscribe->payload,
    ...
    err = send(mq_sess->socket, p_buf_subscribe->buf, p_buf_subscribe->len, 0);
    if (0 > err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor send subscribe fail!=\n");
        return err;
    }
    ...
    union mqtt_attr_payload_suback_ret_code ret_code = {.all = ret_code_digit};
    if (MQTT_CTL_TYPE_SUBACK == p_attr_suback->hdr.bits.type) {
        ...
        err = -E_SESS_ACK;
        mqtt_log_printf(LOG_LEVEL_WARN, "Mqttor client don't recv suback!\n");
    }
...
int mqttor_client_unsubscribe(mqttor_session_t* mq_sess, const char* sub) {
    ...
    int err = 0;
    size_t len_sub = strlen(sub);
    mqtt_attr_packet_t* p_attr_unsubscribe =
        mqtt_attr_packet_new(len_sub + MQTT_BUF_STR_MAX_BYTE);
    ...
    if (0 > err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor pack unsubscribe fail!\n");
        return err;
    }
    mqtt_attr_packet_release(p_attr_unsubscribe);
    err = send(mq_sess->socket, p_buf_unsubscribe->buf, p_buf_unsubscribe->len,
               0);
    if (0 > err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor send unsubscribe fail!\n");
        return -E_NET_XFER;
    }
    mqtt_buf_release(p_buf_unsubscribe);

    //< unsuback
    mqtt_buf_t* p_buf_unsuback = mqtt_buf_new(MQTT_FIXED_PACKET_LEN_UNSUBACK);
    err = recv(mq_sess->socket, p_buf_unsuback->buf, p_buf_unsuback->len, 0);
    if (0 > err) {
...
int mqttor_client_pingreq(mqttor_session_t* mq_sess) {
    ...
    //< wait pingresp
    mqtt_buf_packet_t* p_buf_pingresp =
        mqtt_buf_new(MQTT_FIXED_PACKET_LEN_PINGRESP);
