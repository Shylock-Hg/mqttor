...
//
static const char* errstr[] = {"E_NONE", "E_FORMAT_CHECK", "E_MEM_FAIL",
                               "E_MEM_OUT"};

const char* mqtt_str_error(mqtt_err_t err) { return errstr[err]; }
...
struct mqtt_buf_uint16* mqtt_buf_uint16_encode(mqtt_attr_uint16_t num) {
    struct mqtt_buf_uint16* mq_uint16 =
        mqtt_buf_new(sizeof(mqtt_attr_uint16_t));
