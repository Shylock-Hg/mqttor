...
void mqttor_config_release(mqttor_config_t* mq_config) { free(mq_config); }
...
int mqttor_session_on_publish(mqttor_session_t* mq_sess,
    ...
    enum mqtt_ctl_type type = MQTT_BUF_SEGMENT_EVAL(
        buf_packet->buf[0], MQTT_CTL_TYPE_Msk, MQTT_CTL_TYPE_OFFSET);
    ...
    switch (type) {
        case MQTT_CTL_TYPE_PUBLISH:
            ...
            if (0 > err) {
                ...
                if (p_attr_publish) mqtt_attr_packet_release(p_attr_publish);
                return err;
            }
            ...
            switch (p_attr_publish->hdr.bits.QoS) {
                ...
                case MQTTOR_QoS_EONCE:
                    ...
                    /* pubrel */
                    mqtt_buf_t* p_buf_pubrel =
                        mqtt_buf_new(MQTT_FIXED_PACKET_LEN_PUBREL);
