 ...
 * */
//#define MQTT_PACKET_IDENTIFIER_PACK(identifier,bytes)
// UINT16_2_BYTES(identifier,bytes)

/*! \brief check packet type with identifier
 * */
#define MQTT_PACKET_IDENTIFIER_CHECK(mqtt_ctl_type)  \
    assert(MQTT_CTL_TYPE_PUBLISH <= mqtt_ctl_type && \
...
#define MQTT_CONNECT_FLAG_W_QoS_Msk        \
    (BIT(MQTT_CONNECT_FLAG_W_QoS_OFFSET) | \
     BIT(MQTT_CONNECT_FLAG_W_QoS_OFFSET + 1))  //!< two bits
#define MQTT_CONNECT_FLAG_W_FLAG_Msk BIT(MQTT_CONNECT_FLAG_W_FLAG_OFFSET)
#define MQTT_CONNECT_FLAG_CLEAN_SESSION_Msk \
    BIT(MQTT_CONNECT_FLAG_CLEAN_SESSION_OFFSET)
...
#define MQTT_CONNECT_FLAG_UNPACK(p_buf_flag) \
    ((union mqtt_attr_connect_flag)(p_buf_flag->buf[0]))
...
#define MQTT_CONNACK_FLAG_UNPACK(p_buf_flag) \
    ((union mqtt_attr_connack_flag)(p_buf_flag->buf[0]))
...
typedef enum mqtt_attr_connack_ret_code {
    ...
    CONNECT_RET_CODE_REF_VER,   //!< unavailable protocol version suported by
                                //!< server
    CONNECT_RET_CODE_REF_ID,  //!< identifier is format-legal but not allowed by
                              //!< server
...
#define MQTT_CONNECT_RET_CODE_CHECK(conn_ret_code) \
    assert(CONNECT_RET_CODE_RESERVED > conn_ret_code)
