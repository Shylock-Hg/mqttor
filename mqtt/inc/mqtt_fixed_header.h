...
/*! \brief Fixed header
 *         [B0]|control packet type[7:4]|control packet flag specific to each
 * type[3:0]| [B0-B3]|Remaining length|
 *
 * */
 ...
 * */
#define MQTT_CTL_TYPE_CHECK(type)               \
    assert(MQTT_CTL_TYPE_LOWER_BORDER < type && \
...
//#define MQTT_CTL_TYPE_EVAL(byte)     ((byte & MQTT_CTL_TYPE_Msk) >>
// MQTT_CTL_TYPE_OFFSET)
...
//#define MQTT_CTL_FLAG_DUP_EVAL(byte) ((byte & MQTT_CTL_FLAG_DUP_Msk) >>
// MQTT_CTL_FLAG_DUP_OFFSET)
...
//#define MQTT_CTL_FLAG_QoS_EVAL(byte) ((byte & MQTT_CTL_FLAG_QoS_Msk) >>
// MQTT_CTL_FLAG_QoS_OFFSET)
...
//#define MQTT_CTL_FLAG_RETAIN_EVAL(byte)    ((byte & MQTT_CTL_FLAG_RETAIN_Msk)
//>> MQTT_CTL_FLAG_RETAIN_OFFSET)
 ...
 *  \retval the byte buf
 * */
struct mqtt_buf_ctl_flag* mqtt_ctl_flag_pack(
    const union mqtt_attr_ctl_flag flag);

/*! \brief unpack mqtt control packet flags
 *  \param p_buf the pointer to buf
 *  \retval the flags bits field
 * */
union mqtt_attr_ctl_flag mqtt_ctl_flag_unpack_low(
    const struct mqtt_buf_ctl_flag* p_buf_ctl_flag);
/*! \brief unpack mqtt control packet flags
...
#define MQTT_CTL_REMAINING_LEN_CHECK(length) assert(0xFFFFFF7F >= length)
...
mqtt_attr_re_len_t mqtt_ctl_decode_remaining_len_low(
    const struct mqtt_buf_re_len* mq_buf_re_len);
 ...
 * */
mqtt_attr_re_len_t mqtt_ctl_decode_remaining_len(uint8_t* buf,
                                                 size_t* len_bytes);
...
struct mqtt_buf_re_len* mqtt_ctl_encode_remaining_len(
    mqtt_attr_re_len_t mq_attr_re_len);
