...
static int at_cmd_MQSUB_set_handler(char* parameter) {
    ...
    if (err) {
        mqtt_log_printf(LOG_LEVEL_ERR, "Mqttor client subscribe `%s` fail!\n",
                        topic);
    }
...
static at_cmd_cb_t mqttor_client_at[] = {
    ...
    {AT_FLAG_VISIABLE, "MQUNSUB", at_cmd_MQUNSUB_set_handler, NULL, NULL,
     NULL}};
...
int main(int argc, char* argv[]) {
    ...
    while (runcond) {
        ...
        if (mq_sess->is_connected) {  //!< connected to broker and listen to
                                      //!< publish
            //< receive publish packet
            err = recv(mq_sess->socket, buf->buf, buf->len, MSG_DONTWAIT);
            if (0 < err) {  //!< receive packet
                mqtt_log_printf(LOG_LEVEL_LOG, "Mqttor client recv %d bytes!\n",
                                err);
